...
import { Download, Edit, Search, Trash2, X } from "lucide-react";
...
export default function DailyNotes() {
  ...
  const handleUpdateNote = (updatedNote) => {
    fetch(`http://localhost:5000/api/notes/${updatedNote._id}`, {
      ...
      .then((data) => {
        setNotes(
          notes.map((note) => (note._id === updatedNote._id ? data : note))
        );
        setActiveNote(data);
        setIsEditing(false);
      })
...
const exportNoteToPDF = (note) => {
  ...
  doc.setFontSize(18);
  doc.text(note.title || "No name", margin, y);
  y += 10;

  doc.setFontSize(12);

  // rozdel text na riadky podľa šírky (180 je šírka v mm)
  ...
  doc.save(`${note.title ? note.title.replace(/\s+/g, "_") : "note"}.pdf`);
};
            ...
            <div className="h-full flex flex-col">
              <div className="border-b border-gray-200 px-6 py-4 flex justify-between items-center">
                ...
                <div className="flex space-x-2">
                  {isEditing ? (
                    <ReuseButton
                      Sign={<X className="h-4 w-4" />}
                      handleClick={() => setIsEditing(false)}
                    />
                  ) : (
                    <>
                      <ReuseButton
                        Sign={<Edit className="h-4 w-4" />}
                        handleClick={() => setIsEditing(true)}
                      />
                      <ReuseButton
                        Sign={<Trash2 className="h-4 w-4" />}
                        handleClick={() => handleDeleteNote(activeNote._id)}
                      />
                      <ReuseButton
                        Sign={<Download className="h-4 w-4" />}
                        handleClick={() => exportNoteToPDF(activeNote)}
                      />
                    </>
                  )}
                </div>
